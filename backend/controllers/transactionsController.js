...
exports.transferMoney = async (req, res) => {
    ...
    try {
        ...
        const [recipients] = await connection.query('SELECT * FROM users WHERE username = ? FOR UPDATE', [recipientUsername]);
        ...
        await connection.query('INSERT INTO movements (user_id, type, amount, counterparty_username) VALUES (?, ?, ?, ?)', [senderId, 'transfer_out', -transferAmount, recipient.username]);
        await connection.query('INSERT INTO movements (user_id, type, amount, counterparty_username) VALUES (?, ?, ?, ?)', [recipient.id, 'transfer_in', transferAmount, sender.username]);
...
exports.requestLoan = async (req, res) => {
    ...
    try {
        const [deposits] = await pool.query("SELECT * FROM movements WHERE user_id = ? AND amount > ?",[userId, loanAmount * 0.10]);
        if (deposits.length === 0) {
            return res.status(400).json({ message: 'Loan denied. You must have at least one deposit greater than 10% of the loan amount.' });
        }
        ...
        res.json({ message: 'Loan approved!', newBalance: users[0].balance });
    } catch (error) {
        res.status(500).json({ message: 'Server error', error: error.message });
    }
};
